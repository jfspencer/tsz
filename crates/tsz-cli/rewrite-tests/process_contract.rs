...
#[test]
fn cli_syntax_precedes_semantics_in_both_root_orders() {
    let project = tempfile::tempdir().unwrap();
...
#[test]
fn cli_selects_syntax_then_program_then_semantic_diagnostics() {
    let project = tempfile::tempdir().unwrap();
    std::fs::write(
        project.path().join("tsconfig.json"),
...
#[test]
fn cli_syntax_precedes_program_and_global_diagnostics() {
    let project = tempfile::tempdir().unwrap();
...
#[test]
fn program_option_dependencies_precede_checker_diagnostics() {
    let project = tempfile::tempdir().unwrap();
    let source = project.path().join("case.ts");
    let diagnostics_path = project.path().join("diagnostics.json");
    std::fs::write(
